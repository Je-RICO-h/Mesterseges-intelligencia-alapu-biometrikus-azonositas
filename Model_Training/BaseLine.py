...
def preprocess_and_scale_data(file_path):
    ...
    #Separate the label and session id columns
    df_labels = df_encoded[USER_ID_COLUMN]
    df_session_ids = df_encoded['session_id']
    df_features = df_encoded.drop(columns=[USER_ID_COLUMN, 'session_id'])
    ...
    main_mask = df_processed[USER_ID_COLUMN] == MAIN_USER_LABEL
    main_user_data = df_processed[main_mask].drop(USER_ID_COLUMN, axis=1)
    other_users_data = df_processed[~main_mask].drop(USER_ID_COLUMN, axis=1)
    ...
    Y_labels_raw = pd.concat([
        df_processed[main_mask][USER_ID_COLUMN],
        df_processed[~main_mask][USER_ID_COLUMN]
    ]).values
    session_ids_raw = pd.concat([
        df_session_ids[main_mask],
        df_session_ids[~main_mask]
    ]).values

    N_FEATURES_PER_TIMESTEP = X_scaled.shape[1]

    return X_scaled, Y_labels_raw, session_ids_raw, N_FEATURES_PER_TIMESTEP, scaler, final_feature_list


def create_short_sequences(user_data, user_label, session_id, sequence_length = TARGET_SEQUENCE_LENGTH):
    """
...
def read_and_prepare_keystroke_data_full(file_path):
    X_scaled_flat, Y_labels_raw, session_ids_raw, N_features, scaler_obj, feature_list_obj = preprocess_and_scale_data(file_path)

    df_temp = pd.DataFrame(X_scaled_flat)
    df_temp[USER_ID_COLUMN] = Y_labels_raw
    df_temp['session_id'] = session_ids_raw

    X_sequences_segmented = []
    Y_labels_segmented = []
    session_ids_segmented = []

    for (user_name, session_id), group_df in df_temp.groupby([USER_ID_COLUMN, 'session_id']):
        user_data_matrix = group_df.drop(columns=[USER_ID_COLUMN, 'session_id']).values.astype(np.float32)

        segmented_sequences, segmented_labels, segmented_session_ids = create_short_sequences(
            user_data_matrix, user_name, session_id
        )

        X_sequences_segmented.extend(segmented_sequences)
        Y_labels_segmented.extend(segmented_labels)
        session_ids_segmented.extend(segmented_session_ids)
    ...
    session_ids_segmented = np.array(session_ids_segmented)

    return X_padded, Y_labels_encoded, session_ids_segmented, label_encoder, M_max, N_features, scaler_obj, feature_list_obj
...
if __name__ == '__main__':
    ...
    try:
        X_data, Y_data, session_ids, label_encoder, M_max, N_features, scaler_obj, feature_list_obj = \
        ...
        #Split the data by session, so no session appears in both train and test
        gss = GroupShuffleSplit(n_splits=1, test_size=0.2, random_state=42)
        train_idx, test_idx = next(gss.split(X_data, Y_data, groups=session_ids))

        X_train, X_test = X_data[train_idx], X_data[test_idx]
        Y_train, Y_test = Y_data[train_idx], Y_data[test_idx]

        train_sessions = set(session_ids[train_idx])
        test_sessions = set(session_ids[test_idx])
        assert not (train_sessions & test_sessions)

        train_sessions_arr = session_ids[train_idx]
        gss_val = GroupShuffleSplit(n_splits=1, test_size=0.2, random_state=42)
        fit_idx, val_idx = next(gss_val.split(X_train, Y_train, groups=train_sessions_arr))

        X_fit, X_val = X_train[fit_idx], X_train[val_idx]
        Y_fit, Y_val = Y_train[fit_idx], Y_train[val_idx]

        fit_sessions = set(train_sessions_arr[fit_idx])
        val_sessions = set(train_sessions_arr[val_idx])
        assert not (fit_sessions & val_sessions)

        print(f"\nTraining Samples: {X_fit.shape[0]}, Validation Samples: {X_val.shape[0]}, Testing Samples: {X_test.shape[0]}, Classes: {num_users}")
